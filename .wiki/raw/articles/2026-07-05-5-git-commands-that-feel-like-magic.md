---
title: "5 git commands that feel like magic"
source: "https://www.howtogeek.com/git-commands-that-feel-like-magic/"
type: articles
ingested: 2026-07-05
tags: [git, developer-tools, version-control, command-line-tools]
summary: "How-To Geek article explaining useful Git commands including git blame, git archive, git stash, git grep, and git worktree."
---

# [5 git commands that feel like magic](https://www.howtogeek.com/git-commands-that-feel-like-magic/)

The git program supports so many sub-commands that itâ€™s almost impossible to keep on top of them all. Most of them are quite low-level and carry out simple tasks like committing code changes or switching branch. But some of the most obscure offering surprising functionality that does way beyond the norm.

## git blame: find out who changed what

Personally, I think git blame could use a more positive name: git credit, maybe? Anyway, however you choose to use it, this command will tell you exactly whoâ€™s responsible for each line of code in a file:
...
What youâ€™ll see by default is every line in your file, prefixed with details of the last commit that changed it. Those details include the commit ID, the date and time of that commit, and the author who made that change.

If youâ€™re working on a larger project with many contributors, this can be a valuable way of following up on specific code changes. You can use it to find out why somebody made them, or consult on a possible update.
...
Blame is a feature that GitHub supports too: navigate to any individual file, and youâ€™ll see a â€œBlameâ€ tab next to the default, â€œCode:â€
...
Working with a Git project typically involves cloning repositories, checking out branches, and committing changes. But, sometimes, you want to work with just the project files: when youâ€™re running a demo or carrying out casual testing, for example.
...
This command outputs the contents of all files in the current branch, in tar format. Youâ€™ll probably want to save that output, and there are two ways to do so. You can use shell file redirection to send the output to a file:
...
Time and time again, Iâ€™ve run into this situation: youâ€™re busy [working on a branch](https://www.howtogeek.com/devops/how-do-git-branches-work/), then something else needs attention. Maybe itâ€™s a feature that needs tweaking or a branch that requires a quick bug fix. Youâ€™ve saved your files, but switching branches now will remove those changes.

When I didnâ€™t know any better, I would typically check in my changes, hoping to clean up the commit history later (and probably failing). But this requires a lot of admin and leaves the repository in a messy state, even if just for a brief time. But now I know about git stash:
...
Once youâ€™ve run git stash, youâ€™ll be left with a clean working tree, so you can safely switch branches and work on something else. When youâ€™re ready to return, you can restore your files:
...
The git grep sub-command searches through files and prints lines that match a pattern. That might not sound too impressive at first; after all, isnâ€™t that [exactly what grep does](https://www.howtogeek.com/496056/how-to-use-the-grep-command-on-linux/)? The magical bit is how well Git integrates this alternative grep.
...
Without git grep, youâ€™d have to check out the specific revision before grepping and account for ignored files, untracked files, and recursion. You can even search through multiple revisions, including the entire history of your repo, if youâ€™re feeling brave:
...
If you thought git stash was good, hereâ€™s something even better: git worktree. While stashing is fine for short-term work, it can get awkward if youâ€™re working on files over a longer period or from many different branches at once.
...
As with most Git features, you can adapt worktrees to fit your process, but hereâ€™s a simple example of how to get started:
