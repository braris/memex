...
source: "https://machinelearningmastery.com/building-ai-agents-with-local-small-language-models/"
type: articles
ingested: 2026-07-05
tags: [ai-agents, small-language-models, local-ai, ollama, langchain, langgraph, privacy, offline-ai]
summary: "MachineLearningMastery tutorial on building local AI agents with small language models using Ollama, LangChain, LangGraph, tools, ReAct-style agent execution, and conversation memory, with notes on privacy, offline use, and limitations."
fetched: 2026-07-05
---
...
[Small language models (SLMs)](https://huggingface.co/blog/smollm) are AI models trained on large amounts of text data â€” similar to large models like GPT-4 â€” but designed to be much more lightweight.
...
Now for the exciting part. Let us build a simple agent that can answer questions and use a basic tool â€” a calculator.
...
- The `create_react_agent` function uses the [ReAct pattern](https://react-lm.github.io/) â€” a method where the agent reasons about the problem and then acts using a tool, repeatedly, until it has an answer.
...
You will see the agentâ€™s thought process printed in the terminal before it produces the final answer.

## Adding Memory and Tools to Your Agent

A real agent needs to remember what was said earlier in a conversation. Here is how to add conversation memory and a second tool â€” a simple knowledge base lookup.
...
- Smaller models make more mistakes. SLMs are not as capable as GPT-4 or Claude. They can hallucinate â€” confidently give wrong answers â€” more often, especially on complex tasks.
- Speed depends on your hardware. If you do not have a GPU, your model may run slowly. Expect 5â€“30 seconds per response depending on your machine.
- Context length is limited. Most SLMs can only handle shorter conversations before they â€œforgetâ€ earlier messages. This is a known limitation of smaller models.
