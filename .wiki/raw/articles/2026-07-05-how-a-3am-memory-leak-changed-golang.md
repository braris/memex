---
title: "How a 3AM memory leak changed the way I look at Golang forever"
source: "https://medium.com/@nandoseptian/how-a-3am-memory-leak-changed-the-way-i-look-at-golang-forever-972c1530cdd2"
type: articles
ingested: 2026-07-05
tags: [go, golang, memory-leaks, pprof, observability, production-debugging]
summary: "Medium article about Go memory forensics, covering pprof, heap snapshots, goroutine leak detection, map cleanup, slice preallocation, and production memory monitoring."
---
...
They say programming is an art. If thatâ€™s true, then debugging memory leaks in production is a never-ending horror movie. After 2 years of developing Go applications from simple APIs to complex VR counseling systems, one thing I learned the hard way: memory forensics isnâ€™t optional â€” itâ€™s survival.

This article isnâ€™t another basic pprof tutorial youâ€™ll find everywhere else. Itâ€™s a compilation of memory investigation techniques I developed after facing production nightmares that almost made me switch to JavaScript (yes, I was that desperate).
...
## Goâ€™s Memory Model: What They Donâ€™t Teach You

After that incident, I spent weeks diving deep into Goâ€™s memory internals. It turns out there are many misconceptions floating around in the developer community.

### Garbage Collector Isnâ€™t a Silver Bullet

Go uses a concurrent, non-generational, tri-color mark-and-sweep garbage collector. Sounds sophisticated, but thereâ€™s a catch rarely discussed.

The GC can only reclaim memory thatâ€™s truly unreachable. If thereâ€™s still a pointer reference, no matter how small, that memory will live forever.
...
Goâ€™s compiler performs escape analysis to determine whether a variable can be allocated on the stack or must go to the heap. But we often donâ€™t realize our code is forcing heap allocations.
...
Standard pprof tutorials only show basic profiling. Whatâ€™s rarely taught: comparative memory analysis.
...
Goroutine leaks are silent killers that often go undetected until itâ€™s too late.
...
Root cause: Goâ€™s aggressive slice growth strategy. Every time capacity runs out, Go allocates a new slice with 2x capacity. For large datasets, this resulted in massive memory waste.
...
Goâ€™s GC is great, but not magical. Poor design will remain problematic regardless of how sophisticated the GC is.[freecodecamp](https://www.freecodecamp.org/news/how-i-investigated-memory-leaks-in-go-using-pprof-on-a-large-codebase-4bec4325e192/)
...
Memory forensics is an essential skill. Itâ€™s not optional, but a requirement for production Go applications.
